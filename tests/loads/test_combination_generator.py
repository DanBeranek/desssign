...
from desssign.loads.enums import LoadCaseRelation, LoadType, VariableCategory, LoadDurationClass, LimitState, ULSCombination
...
def test_generate_combinations_basic(
    ...
    for combination in combinations_generator.combinations:
        ...
        assert combination.leading_variable_case in imposed_load_case_group.load_cases + wind_load_case_group.load_cases + [None]
        ...
        cases = (combination.permanent_cases, combination.leading_variable_case, combination.other_variable_cases)
        ...
        assert all(case in combination.load_cases for case in permanent_load_case_group.load_cases)

        # Check that there are 0 or 1 wind load cases in the combination
        wind_cases = [case for case in combination.load_cases if case in wind_load_case_group.load_cases]
        assert len(wind_cases) in [0, 1]

        # Check that there are 0 to n imposed load cases in the combination
        imposed_cases = [case for case in combination.load_cases if case in imposed_load_case_group.load_cases]
...
def test_generate_combinations_alternative(
    ...
    combinations_generator = CombinationsGenerator(LimitState.ULS, ULSCombination.ALTERNATIVE)
    combinations_generator.generate_combinations(
        [
            permanent_load_case_group,
            imposed_load_case_group,
    ...
    for combination in combinations_generator.combinations:
        ...
        assert combination.permanent_cases == permanent_load_case_group.load_cases
        assert combination.leading_variable_case in imposed_load_case_group.load_cases + wind_load_case_group.load_cases + [
            None]
        ...
        cases = (tuple(combination.permanent_cases), combination.leading_variable_case, tuple(combination.other_variable_cases))
    ...
    combinations_generator_basic = CombinationsGenerator(LimitState.ULS, ULSCombination.BASIC)
    ...
    assert len(combinations_generator.combinations) == 2 * len(combinations_generator_basic.combinations)
